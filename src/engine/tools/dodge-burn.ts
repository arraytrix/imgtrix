...
export class DodgeBurnTool implements Tool {
  size     = 40
  hardness = 0.5
  strength = 50   // 1–100
  ...
  private activeMode: DodgeBurnMode = 'dodge'
  ...
  onPointerDown(event: ToolEvent, context: ToolContext): void {
    ...
    this.lastX = event.x
    this.lastY = event.y
    this.activeMode = this.mode === 'dodge' ? 'burn' : 'dodge'
    this.dab(event.x, event.y, context)
  }
  ...
  onPointerUp(_event: ToolEvent, context: ToolContext): HistoryEntry | null {
    ...
    const entry: HistoryEntry = {
      description: this.activeMode === 'dodge' ? 'Lighten' : 'Darken',
  ...
  private dab(cx: number, cy: number, context: ToolContext): void {
    ...
    for (let row = 0; row < rh; row++) {
      for (let col = 0; col < rw; col++) {
        ...
        if (this.activeMode === 'dodge') {
          // Lighten: mix each channel toward 255
          pix[pi]     = rr + (255 - rr) * amt
          pix[pi + 1] = gg + (255 - gg) * amt
          pix[pi + 2] = bb + (255 - bb) * amt
        } else {
          // Darken: mix each channel toward 0
          pix[pi]     = rr * (1 - amt)
          pix[pi + 1] = gg * (1 - amt)
          pix[pi + 2] = bb * (1 - amt)
        }
